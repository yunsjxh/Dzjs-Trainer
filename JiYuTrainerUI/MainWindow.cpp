...
const wchar_t* kToggleLabels[] = {
	...
	L"\u5141\u8bb8\u6559\u5e08\u67e5\u770b\u5c4f\u5e55", L"\u963b\u6b62\u8fdc\u7a0b\u5173\u95ed\u7a97\u53e3", L"\u5141\u8bb8\u6559\u5e08\u63a7\u5236\u7535\u8111", L"\u81ea\u52a8\u68c0\u67e5\u66f4\u65b0",
	L"\u89c6\u9891\u6d41\u4fdd\u62a4"
};
...
constexpr wchar_t kAvPromptClass[] = L"JiYuTrainerAvTextPrompt";
...
bool PromptAvText(HWND owner, LPCWSTR title, LPCWSTR prompt, std::wstring* value)
{
	...
	HWND window = CreateWindowExW(WS_EX_DLGMODALFRAME | WS_EX_APPWINDOW, kAvPromptClass,
...
constexpr wchar_t kExitConfirmClass[] = L"JiYuTrainerExitConfirm";
...
bool ConfirmExit(HWND owner, bool darkMode)
{
	...
	swprintf_s(desktopName, L"JiYuTrainerPassword_%lu_%lu",
...
constexpr wchar_t kAvProcessPickerClass[] = L"JiYuTrainerAvProcessPicker";
...
bool ChooseAvProcess(HWND owner, DWORD* processId, std::wstring* processName)
{
	...
	HWND window = CreateWindowExW(WS_EX_DLGMODALFRAME | WS_EX_APPWINDOW, kAvProcessPickerClass,
...
constexpr wchar_t kSamplePickerClass[] = L"JiYuTrainerExeSamplePicker";
...
constexpr wchar_t kLogSavePickerClass[] = L"JiYuTrainerLogSavePicker";
...
struct LogSavePickerContext {
	...
	HFONT font = nullptr;
	std::wstring currentDirectory;
	std::wstring initialName = L"JiYuTrainer-log.txt";
...
bool ChooseLogSavePath(HWND owner, std::wstring* path)
{
	...
	const int y = (std::max)(workArea.top,
		(std::min)(workArea.bottom - height, ownerRect.top + (ownerRect.bottom - ownerRect.top - height) / 2));
	HWND window = CreateWindowExW(WS_EX_DLGMODALFRAME | WS_EX_APPWINDOW,
		kLogSavePickerClass, L"导出日志", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_SIZEBOX,
		x, y, width, height, nullptr, nullptr, JTAppGetInstanceDirect(), &context);
	if (!window) return false;
	const BOOL ownerWasEnabled = owner && IsWindowEnabled(owner);
	if (ownerWasEnabled) EnableWindow(owner, FALSE);
	ShowWindow(window, SW_SHOW);
...
constexpr wchar_t kMediaPickerClass[] = L"JiYuTrainerMediaPicker";
...
bool ChooseMediaPath(HWND owner, bool image, std::wstring* path)
{
	...
	const int x = work.left + ((work.right - work.left) - width) / 2;
	const int y = work.top + ((work.bottom - work.top) - height) / 2;
	HWND window = CreateWindowExW(WS_EX_DLGMODALFRAME | WS_EX_APPWINDOW, kMediaPickerClass,
...
bool MainWindow::IsProtectedInputTarget(HWND target, DWORD* targetPid)
{
	...
	if (pid == 0 || pid == GetCurrentProcessId() || pid == inputGuardStudentPid) return pid != 0;

	HANDLE process = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, pid);
	if (!process) return false;
	wchar_t imagePath[MAX_PATH]{};
	DWORD imageChars = _countof(imagePath);
	const BOOL queried = QueryFullProcessImageNameW(process, 0, imagePath, &imageChars);
	CloseHandle(process);
	if (!queried) return false;
	const wchar_t* fileName = wcsrchr(imagePath, L'\\');
	fileName = fileName ? fileName + 1 : imagePath;
	return _wcsicmp(fileName, L"StudentMain.exe") == 0;
}
...
bool MainWindow::InstallInputOriginHooks()
{
	...
	inputGuardStudentPid = studentPid;
...
LRESULT CALLBACK MainWindow::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_BLOCKED_INJECTED_INPUT: {
		InterlockedExchange(&inputGuardNoticePending, 0);
		const bool ownWindow = static_cast<DWORD>(lParam) == GetCurrentProcessId();
		MessageBoxW(hwnd,
			wParam ? L"\u5df2\u62e6\u622a\u9488\u5bf9\u53d7\u4fdd\u62a4\u7a97\u53e3\u7684 API \u6a21\u62df\u9f20\u6807\u8f93\u5165\u3002" : L"\u5df2\u62e6\u622a\u9488\u5bf9\u53d7\u4fdd\u62a4\u7a97\u53e3\u7684 API \u6a21\u62df\u952e\u76d8\u8f93\u5165\u3002",
			ownWindow ? L"Dzjs Trainer \u8f93\u5165\u4fdd\u62a4" : L"StudentMain \u8f93\u5165\u4fdd\u62a4",
...
void MainWindow::PaintNavigation(Graphics& g, int height)
{
	...
	Text(g, L"v1.7.6  \u00b7  \u672c\u673a", 54, static_cast<float>(height - 45), 136, 16, 9, FontStyleRegular, C(d, 91, 99, 95, 166, 177, 171));
}
...
void MainWindow::PaintOverview(Graphics& g, int width, int height, int offsetY)
{
	...
	const wchar_t* labels[] = { L"\u9632\u62a4\u72b6\u6001", topMostUiAccessBand ? L"\u8d85\u7ea7\u7f6e\u9876" : L"\u7a97\u53e3\u7f6e\u9876", studentRunning ? L"\u7ed3\u675f\u6781\u57df" : L"\u91cd\u542f\u6781\u57df", L"\u63a7\u5236\u4e0e\u8bbe\u7f6e", L"\u7535\u6e90\u63a7\u5236" };
...
void MainWindow::PaintProtection(Graphics& g, int width, int height, int offsetY)
{
	...
	Text(g, temporaryVideoProtection ? L"已启用" : L"已关闭", x + w - 134, panelY + 23, 92, 18, 10, FontStyleBold,
	...
	temporaryVideoImageModeRect = { static_cast<LONG>(controlX), static_cast<LONG>(panelY + 66), static_cast<LONG>(controlX + 76), static_cast<LONG>(panelY + 98) };
	temporaryVideoFileModeRect = { static_cast<LONG>(controlX + 84), static_cast<LONG>(panelY + 66), static_cast<LONG>(controlX + 160), static_cast<LONG>(panelY + 98) };
	auto drawMode = [&](RECT rect, LPCWSTR label, bool selected) {
		FillRound(g, RectF(static_cast<REAL>(rect.left), static_cast<REAL>(rect.top), static_cast<REAL>(rect.right - rect.left), static_cast<REAL>(rect.bottom - rect.top)), 16,
			selected ? C(darkMode, 183, 232, 222, 42, 79, 72) : C(darkMode, 225, 232, 228, 39, 47, 44));
		Text(g, label, static_cast<float>(rect.left), static_cast<float>(rect.top + 8), static_cast<float>(rect.right - rect.left), 18, 9,
	...
	temporaryVideoImagePickerRect = { static_cast<LONG>(controlX), static_cast<LONG>(panelY + 108), static_cast<LONG>(controlX + 116), static_cast<LONG>(panelY + 142) };
	temporaryVideoFilePickerRect = { static_cast<LONG>(controlX + 124), static_cast<LONG>(panelY + 108), static_cast<LONG>(controlX + 240), static_cast<LONG>(panelY + 142) };
	auto drawPicker = [&](RECT rect, LPCWSTR label, bool selected) {
		...
		Text(g, label, static_cast<float>(rect.left), static_cast<float>(rect.top + 8), static_cast<float>(rect.right - rect.left), 18, 9, FontStyleBold,
	...
	temporaryVideoLoopRect = { static_cast<LONG>(controlX + 248), static_cast<LONG>(panelY + 108), static_cast<LONG>(controlX + 326), static_cast<LONG>(panelY + 142) };
	FillRound(g, RectF(controlX + 248, panelY + 108, 78, 34), 7,
		temporaryVideoLoop ? C(darkMode, 183, 232, 222, 42, 79, 72) : C(darkMode, 225, 232, 228, 39, 47, 44));
	Text(g, temporaryVideoLoop ? L"循环：开" : L"循环：关", controlX + 248, panelY + 116, 78, 18, 9, FontStyleBold,
	...
	Text(g, selectedPath.empty() ? (imageMode ? L"尚未选择图片" : L"尚未选择视频") : selectedPath.c_str(), controlX, panelY + 150, w - 282, 18, 8, FontStyleRegular,
		C(darkMode, 91, 99, 95, 166, 177, 171));

	const float previewW = 156;
	const float previewH = (std::max)(92.0f, panelH - 28.0f);
	temporaryVideoPreviewRect = { static_cast<LONG>(x + w - previewW - 42), static_cast<LONG>(panelY + 14), static_cast<LONG>(x + w - 42), static_cast<LONG>(panelY + 14 + previewH) };
	...
	if (temporaryVideoPreview && temporaryVideoPreview->GetLastStatus() == Ok) {
		...
		g.DrawImage(temporaryVideoPreview.get(), x + w - previewW - 42 + (previewW - drawW) / 2.0f, panelY + 14 + (previewH - drawH) / 2.0f, drawW, drawH);
	}
	else {
		DrawIcon(g, imageMode ? 3 : 5, x + w - previewW / 2.0f - 52, panelY + previewH / 2.0f - 3, C(darkMode, 135, 156, 148, 116, 140, 130), 24);
		Text(g, imageMode ? L"图片预览" : L"视频文件", x + w - previewW - 42, panelY + previewH / 2.0f + 28, previewW, 18, 9, FontStyleRegular,
			C(darkMode, 135, 156, 148, 116, 140, 130), StringAlignmentCenter);
	}
	advancedSettingsRect = { static_cast<LONG>(x + w - 250), static_cast<LONG>(panelY + 14), static_cast<LONG>(x + w - 144), static_cast<LONG>(panelY + 48) };
	FillRound(g, RectF(x + w - 250, panelY + 14, 106, 34), 17, C(darkMode, 219, 238, 232, 43, 76, 69));
	Text(g, L"高级设置", x + w - 250, panelY + 23, 106, 18, 9, FontStyleBold, C(darkMode, 0, 81, 72, 177, 239, 228), StringAlignmentCenter);
}
...
void MainWindow::PaintAbout(Graphics& g, int width, int height, int offsetY)
{
	...
	Text(g, L"\u7248\u672c  1.7.6.0515", x + 42, y + 166, 142, 18, 10, FontStyleBold, C(darkMode, 0, 81, 72, 177, 239, 228), StringAlignmentCenter);
...
void MainWindow::UpdateHover(POINT point)
{
	...
	if (page == Page::About && Hit(aboutExitRect, point)) target = 73;
...
void MainWindow::HandleClick(POINT point)
{
	...
	if (page == Page::Overview) {
		for (int i = 0; i < 5; ++i) if (Hit(actionRects[i], point)) {
			...
			case 1:
			{
				...
				setTopMost = target;
				topMostUiAccessBand = target && uiAccessBandApplied;
				ShowFastTip(!target ? L"\u5df2\u53d6\u6d88\u7f6e\u9876" : (topMostUiAccessBand ? L"\u5df2\u542f\u7528\u8d85\u7ea7\u7f6e\u9876" : L"\u5df2\u542f\u7528\u6807\u51c6\u7f6e\u9876"));
				break;
			}
	...
	if (page == Page::Protection) {
		...
		if (Hit(driverUnloadRect, point)) {
			JTAppRunOperationDirect(AppOperationUnLoadDriver);
			ShowFastTip(XDriverLoaded() ? L"\u9a71\u52a8\u5378\u8f7d\u5931\u8d25" : L"\u9a71\u52a8\u5df2\u5378\u8f7d");
			InvalidateRect(_hWnd, nullptr, FALSE);
			return;
		}
		for (int i = 0; i < 8; ++i) if (Hit(toggleRects[i], point)) {
			switch (i) {
			...
			case 7: setAutoUpdate = !setAutoUpdate; break;
			}
			SaveSettings();
			InvalidateRect(_hWnd, nullptr, FALSE);
			return;
...
void MainWindow::ExportLogs()
{
	...
	swprintf_s(defaultName, L"JiYuTrainer-log-%04d%02d%02d-%02d%02d%02d.txt",
...
void MainWindow::LoadSettings()
{
	...
	temporaryVideoMode = L"image";
	temporaryVideoImagePath.clear();
	temporaryVideoFilePath.clear();
	temporaryVideoLoop = true;
	temporaryVideoPreview.reset();
}
...
void MainWindow::OnFirstShow()
{
	if (!firstShow) return;
	firstShow = false;
	hotkeyShowHide = GlobalAddAtomW(L"JiYuTrainerShowHide");
	hotkeySwFull = GlobalAddAtomW(L"JiYuTrainerFakeFull");
	UINT mod = 0, key = 0;
	auto settings = JTAppGetSettingsDirect();
	...
	if (setTopMost && !ApplyHighestPermittedTopmost(_hWnd, true, &topMostUiAccessBand)) {
		setTopMost = false;
