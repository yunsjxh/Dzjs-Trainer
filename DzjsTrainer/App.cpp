...
#include <dbghelp.h>
#include <locale>
#include "../DzjsTrainerUI/MainWindow.h"
...
// The UI used to rebuild the ini path itself and hard-coded the file name, which
// diverges from the path the settings object actually uses whenever the exe is
// renamed or started with -f. Expose the real one instead.
extern "C" LPCWSTR JTAppGetIniPathDirect()
{
	auto app = static_cast<JTAppInternal*>(currentApp);
	return app ? app->JTAppInternal::GetPartFullPath(PART_INI) : nullptr;
}

extern "C" LPCWSTR JTAppGetCurrentDirDirect()
{
	auto app = static_cast<JTAppInternal*>(currentApp);
	return app ? app->JTAppInternal::GetCurrentDir() : nullptr;
}

extern "C" LPCWSTR JTAppGetSourceInstallerPathDirect()
{
	auto app = static_cast<JTAppInternal*>(currentApp);
	return app ? app->JTAppInternal::GetSourceInstallerPath() : nullptr;
}
...
int JTAppInternal::CheckAndInstall()
{
	...
	if (!appIsInstaller && !appForceIntallInCurrentDir && SysHlp::CheckIsPortabilityDevice(fullDir.c_str()))
	{
		...
		wcscat_s(szTempPath, L"\\DzjsTrainer");
		...
		wcscat_s(szTempMainStartBatPath, L"\\DzjsTrainerStart.bat");
		//复制本体
	...
	// An updater is a renamed copy of the main executable. It must not stage or
	// load protection modules before replacing the main executable: doing so can
	// crash while the downloaded updater is repeatedly launched.
	if (!appIsInstaller) {
	...
	if (!ResourceMatchesFile(hInstance, MAKEINTRESOURCE(IDR_DLL_HOOKS), L"BIN", fullHookerPath.c_str())) {
		...
		if (InstallResFile(hInstance, MAKEINTRESOURCE(IDR_DLL_HOOKS), L"BIN", stagedHookPath.c_str()) != EXTRACT_RES::ExtractSuccess ||
			...
			appLogger->LogError2(L"Unable to install embedded DzjsTrainerHooks.dll: %s (%d)", PRINT_LAST_ERROR_STR);
			DeleteFile(stagedHookPath.c_str());
			return -1;
		}
		appLogger->LogInfo(L"已释放内嵌 DzjsTrainerHooks.dll");
	}
	...
	// 两个名字都要清：JiYuTrainerDriver.sys 是改名前旧版留下的，
	// DzjsTrainerDriver.sys 是改名后固定文件名版本可能留下的。
	{
		const wchar_t* const legacyNames[] = { L"\\JiYuTrainerDriver.sys", L"\\DzjsTrainerDriver.sys" };
		for (const wchar_t* legacyName : legacyNames) {
			std::wstring legacyDriverPath = fullDir + legacyName;
...
int JTAppInternal::Run(int nCmdShow)
{
	...
	// DzjsTrainerUI destroys MainWindow before returning here. Its live-log
...
int JTAppInternal::RunCheckRunningApp()//如果程序已经有一个在运行，则返回true
{
	...
	const bool nativePreviewBuild = wcsstr(modulePath, L"DzjsTrainer-TeacherNative") != nullptr;
...
RUN_MAIN:
	...
	if (appCrashTestMode)return DzjsTrainerUICommonEntry(4);
	if (appStartType == AppStartTypeNormal) return DzjsTrainerUICommonEntry(0);
	else if (appStartType == AppStartTypeUpdater)  return DzjsTrainerUICommonEntry(1);
	else if (appStartType == AppStartTypeConfig) return DzjsTrainerUICommonEntry(2);
	else if (appStartType == AppStartTypeBugReport)  return DzjsTrainerUICommonEntry(3);
...
LPVOID JTAppInternal::RunOperation(AppOperation op)
{
	...
	case AppOperationUnLoadDriver: {
		const BOOL unloaded = XUnLoadDriver();
		if (unloaded)
			currentLogger->Log(L"驱动卸载成功");
		else
			currentLogger->LogError(L"驱动卸载失败：%lu", static_cast<unsigned long>(GetLastError()));
		return unloaded ? reinterpret_cast<LPVOID>(1) : nullptr;
	}
...
void JTAppInternal::MergePathString()
{
	...
	fullHookerPath = fullDir + L"\\DzjsTrainerHooks.dll";
}
void JTAppInternal::InitPath()
{
	...
	appIsInstaller = Path::GetFileName(fullPath) == L"DzjsTrainerUpdater.exe";
