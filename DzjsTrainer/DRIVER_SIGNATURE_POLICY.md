...
Before DzjsTrainer asks the Service Control Manager to start a kernel driver,
...
This policy controls drivers started by DzjsTrainer. System-wide driver
admission remains the responsibility of Windows Code Integrity or WDAC.

## Kernel driver guard (last-resort blocking)

The user-mode allowlist above only gates drivers that DzjsTrainer itself
starts. The driver guard extends the same publisher allowlist to every
kernel driver loaded by *any* component after DzjsTrainer runs, as a
...
1. On connect, `DriverGuard.cpp` evaluates each candidate in this order:
   ...
   `HKLM\SOFTWARE\DzjsTrainer\DriverPublisherAllowlist`; finally the file
   MD5 is checked against `HKLM\SOFTWARE\DzjsTrainer\DriverMd5Allowlist`.
