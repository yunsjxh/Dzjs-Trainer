...
static BOOL StartDriverHeartbeat()
{
	...
	if (swprintf_s(pulseEventName,
		L"Global\\JiYuTrainerHeartbeatPulse_%lu",
...
static BOOL StartDriverWatchdog()
{
	...
	if (GetModuleFileNameW(NULL, executable, _countof(executable)) == 0)
		return FALSE;
	if (swprintf_s(stopEventName, L"Global\\JiYuTrainerHeartbeatStop_%lu", processId) < 0)
		return FALSE;
	if (swprintf_s(pulseEventName, L"Global\\JiYuTrainerHeartbeatPulse_%lu", processId) < 0)
		return FALSE;
...
size_t separator = binaryPath.rfind(L'\\');
	...
	if (_wcsicmp(fileName.c_str(), L"JiYuTrainerDriver") == 0 ||
...
//  - 旧版固定名 JiYuTrainerDriver / JiYuAvKernel（含正在运行、需卸载授权的自保护实例）
//  - 跨天随机名变更后遗留的孤儿服务
